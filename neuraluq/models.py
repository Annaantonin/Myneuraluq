...
class Model:
    ...
    def _compile_ens(self, method):
        ...
        def losses_fn():
            all_losses = [loss.loss(training=True) for loss in self.likelihoods]
            regularization = [tf.reduce_sum(p.posterior.losses) for p in self.processes]
            return all_losses+ regularization
        # tf.reduce_sum(losses) + tf.reduce_sum(regularization)

        # compile the method
        method.make_sampler(self.sess, loss_fn, losses_fn, self.trainable_variables)
        # assign the method
        self.method = method
